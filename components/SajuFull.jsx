...
const IDOLS={'甲':[{name:'Jin',group:'BTS'},{name:'Rosé',group:'BLACKPINK'}],'乙':[{name:'G-Dragon',group:'BIGBANG'},{name:'V',group:'BTS'}],'丙':[{name:'Jungkook',group:'BTS'},{name:'Nayeon',group:'TWICE'}],'丁':[{name:'Jimin',group:'BTS'},{name:'IU',group:'Solo Artist'}],'戊':[{name:'Lisa',group:'BLACKPINK'}],'己':[{name:'Suga',group:'BTS'},{name:'Karina',group:'aespa'}],'庚':[{name:'Lee Junho',group:'2PM'}],'辛':[{name:'RM',group:'BTS'},{name:'Hyun Bin',group:'Actor'}],'壬':[{name:'Jennie',group:'BLACKPINK'},{name:'Baekhyun',group:'EXO'}],'癸':[{name:'Zico',group:'Block B'},{name:'Taeyang',group:'BIGBANG'}]};
...
function buildPrompt(type,YP,MP,DP,gender){
  ...
  if(type==='story'){const hero=gender==='male'?'a Knight on a quest for love and destiny':'a Princess awakening to her true power';return`You are a master storyteller. Write a Korean fairy tale for adults — beautiful, emotional, easy to follow. The reader is ${hero}.\n\n${base}\nSTORY RULES:\n- Exactly 950-1050 words\n- Second person "you" throughout\n- CLEAR structure: Beginning → Middle → End. Each section should feel distinct.\n- Short sentences. Simple words. Every sentence should be easy to read.\n- Emotional beats: wonder → loss → courage → love. The reader should FEEL each one.\n- Korean imagery: lanterns glowing orange, jade mountains, ancient stone gates, cherry blossoms falling, moonlight on water, silk robes, incense smoke\n- NO markdown at all: no # headers, no --- lines, no **bold**, no bullet points\n- Separate every paragraph with a blank line (two newlines between each paragraph)\n- 4-6 paragraphs per section, each paragraph 3-5 sentences\n\nBEGINNING (기 · Rising — about 300 words):\nOpen with the world: ${eW[YP.se]}. Paint it vividly in 2-3 sentences. Then introduce the hero — who they are, what gift they carry, what they quietly long for. Make the reader fall in love with the hero immediately. End this section with the hero setting out, or a sense that something is about to change.\n\nMIDDLE (승 · The Trial — about 350 words):\nThe trial arrives: ${trials[MP.se]}. Do not rush this. Let the reader feel the weight of it. Show the hero alone, confused, stripped of what they thought they needed. This is the darkest moment. But somewhere in the darkness, show a small flicker — a light, a voice, a memory — that the hero holds onto. End this section with the hero making a choice to keep going, even without knowing where it leads.\n\nENDING (전결 · Destiny — about 350 words):\nThe resolution: ${resols[DP.se]}. Something unexpected and beautiful happens. The hero arrives somewhere — or someone arrives to them. The meeting should feel inevitable, like the universe planned it all along. Write the reunion or arrival with warmth and wonder. End with 2-3 sentences that feel like a exhale — peace, belonging, home. The last line should be the most beautiful line in the story.\n\nIMPORTANT: This story is written for THIS EXACT PERSON based on their birth chart. Weave their specific elements (${YP.se} year, ${MP.se} month, ${DP.se} day) into the imagery naturally — not as labels, but as the textures of their world.`;}
}
    ...
    <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(6,12,24,.95)',zIndex:999,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
      <div style={{background:V.s,border:`1px solid ${V.am}`,padding:'40px 32px',maxWidth:440,width:'100%',textAlign:'center',position:'relative'}}>
        ...
        <div style={{display:'flex',alignItems:'baseline',justifyContent:'center',gap:12,marginBottom:24}}>
          <span style={{fontSize:16,color:'#3A5070',textDecoration:'line-through'}}>$19.99</span>
          <span style={{fontSize:44,color:V.am,fontWeight:700,lineHeight:1}}>$13.99</span>
        </div>
